...
async function handleAddProductForm() {
  ...
  const updatePackageVolume = ({ preferExisting = false } = {}) => {
    ...
    if ([lengthValue, widthValue, heightValue].every(value => value !== null)) {
      const volumeCbm = (lengthValue * widthValue * heightValue) / 1_000_000;
      if (Number.isFinite(volumeCbm)) {
        packageVolumeInput.value = formatCbmValue(volumeCbm);
        return;
      }
    }
  ...
  if (editingId) {
    ...
    const inventoryFields = {
      ...
      packageVolumeCbm: packageVolumeInput
    };
    ...
    Object.entries(inventoryFields).forEach(([key, input]) => {
      ...
      if (key === 'packageVolumeCbm') {
        if (value === '' || value === null || typeof value === 'undefined') {
          const legacyValue = inventory?.packageVolumeCm3;
          if (legacyValue !== null && typeof legacyValue !== 'undefined' && legacyValue !== '') {
            const numericLegacy = Number.parseFloat(legacyValue);
            if (Number.isFinite(numericLegacy)) {
              value = formatCbmValue(numericLegacy / 1_000_000);
            } else {
              value = legacyValue;
            }
          }
          ...
          if (Number.isFinite(numericValue)) {
            value = formatCbmValue(numericValue);
          }
        }
      }
  ...
  form.addEventListener('submit', async event => {
    ...
    const computedVolume = (() => {
      ...
      if ([lengthNumeric, widthNumeric, heightNumeric].every(value => value !== null)) {
        const volumeCbm = (lengthNumeric * widthNumeric * heightNumeric) / 1_000_000;
        if (Number.isFinite(volumeCbm)) {
          const formatted = formatCbmValue(volumeCbm);
          return formatted;
        }
      }

      return (formData.get('packageVolumeCbm') ?? '').toString().trim();
    })();
    const inventoryData = {
      ...
      packageVolumeCbm: computedVolume
    };
