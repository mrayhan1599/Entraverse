...
async function handleAddProductForm() {
  ...
  const setExchangeRateInputValue = (input, value, options = {}) => {
    if (!input) {
      return null;
    }

    const { autoApplied = true } = options;
    const numeric = parseNumericValue(value);
    if (!Number.isFinite(numeric) || numeric <= 0) {
      delete input.dataset.numericValue;
      input.dataset.autoApplied = autoApplied ? 'true' : 'false';
      input.value = '';
      return null;
    }
  ...
  const syncCurrencyForRow = (row, { currency, fallbackRate, preferFallback = false, force = false } = {}) => {
    if (!row) {
      return;
    }
    ...
    let source = 'none';
    const fallbackNumeric = parseNumericValue(fallbackRate);

    if (preferFallback && Number.isFinite(fallbackNumeric) && fallbackNumeric > 0) {
      appliedRate = fallbackNumeric;
      source = 'fallback';
    }

    if (!Number.isFinite(appliedRate) && normalizedCurrency) {
    ...
    if (!Number.isFinite(appliedRate) && Number.isFinite(fallbackNumeric) && fallbackNumeric > 0) {
      appliedRate = fallbackNumeric;
      source = 'fallback';
    }

    if (exchangeRateInput) {
      const manualOverride = exchangeRateInput.dataset.autoApplied === 'false';
      const hasValue = (exchangeRateInput.value ?? '').toString().trim() !== '';
      const shouldApplyRate = force || preferFallback || !manualOverride || !hasValue;

      if (Number.isFinite(appliedRate) && appliedRate > 0 && shouldApplyRate) {
        const autoApplied = source === 'record' && !preferFallback;
        setExchangeRateInputValue(exchangeRateInput, appliedRate, { autoApplied });
      } else if (!Number.isFinite(appliedRate) && shouldApplyRate) {
        const autoApplied = source === 'record' && !preferFallback;
        setExchangeRateInputValue(exchangeRateInput, '', { autoApplied });
      }
    }
  ...
  function hydratePricingRow(row, initialData = {}, variantDefs = getVariantDefinitions()) {
    ...
    syncCurrencyForRow(row, {
      ...
      fallbackRate,
      preferFallback: Number.isFinite(parseNumericValue(fallbackRate))
    });
  }
  ...
  function createPricingRow(initialData = {}, variantDefs = getVariantDefinitions(), options = {}) {
    ...
    if (currencySelect) {
      currencySelect.addEventListener('change', () => {
        syncCurrencyForRow(row, { currency: currencySelect.value, force: true });
      });
    }
    ...
    if (exchangeRateInput) {
      exchangeRateInput.addEventListener('input', () => {
        const numeric = parseNumericValue(exchangeRateInput.value ?? '');
        if (Number.isFinite(numeric) && numeric > 0) {
          exchangeRateInput.dataset.numericValue = numeric.toString();
        } else {
          delete exchangeRateInput.dataset.numericValue;
        }
        exchangeRateInput.dataset.autoApplied = 'false';
        recalculatePurchasePriceIdr(row);
      });
    }
